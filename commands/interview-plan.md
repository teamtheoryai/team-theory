...
2. Retrieve the scorecard / target and company context — from this conversation, `~~cloud storage`, `~~knowledge base`, `~~email`, `~~ATS`, `~~meeting transcripts`, `~~company data`, `~~CRM`, or `search_portfolio_knowledge`. If there is no scorecard, offer to run `/human-capital:target` first.
